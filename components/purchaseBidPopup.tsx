...
export type ItemOptionWithLowestPriceResponseType = {
  ...
  minSaleBiddingPrice: number;
};

const PurchaseBidPopup: React.FC<BidPopupProps> = ({
  isOpen,
  onClose,
  item,
}) => {
  ...
  const handlePurchase = () => {
    router.push(`/purchase/${item.id}?itemOptionId=${optionData}`);
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-8">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold">구매</h2>
          <button
                ...
                priceData.some((price) => price.itemOption_id === option.id) ? (
                  priceData.map((price) =>
                    price.itemOption_id === option.id ? (
                      ...
                      >
                        {price.minSaleBiddingPrice}
                      </p>
                    ) : null,
                  )
                ) : (
                  <p className="text-sm text-red-500">미입찰</p>
                )
              ) : (
                // priceData가 없는 경우
                <p className="text-sm text-red-500">미입찰</p>
        ...
        <button
          ...
          className={` ${optionData ? 'mt-6 bg-red-500 text-white py-2 px-4 rounded-md w-full' : 'hidden'}`}
        >
          구매
        </button>
      </div>
...
export default PurchaseBidPopup;
