  ...
  const handleFetch = async (status: string, method: string) => {
    ...
    try {
      const response = await fetch(
        `${url}/auctions/me/purchase-bid/${status}`,
        {
          method: 'GET',
          headers: {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex justify-between items-center w-full mb-8 my-8">
        ...
        >
          <h2
            className={`text-2xl font-bold ${selectedTab === 'all' ? 'text-red-500' : 'text-gray-500'}`}
          >
            57
          </h2>
          <p className="text-gray-600">구매 일찰</p>
        </div>
        <div
          className={`flex-1 flex flex-col items-center text-center cursor-pointer ${
        ...
        >
          <h2
            className={`text-2xl font-bold ${selectedTab === 'in-progress' ? 'text-red-500' : 'text-gray-500'}`}
          >
            0
          </h2>
        ...
        >
          <h2
            className={`text-2xl font-bold ${selectedTab === 'completed' ? 'text-red-500' : 'text-gray-500'}`}
          >
            12
          </h2>
      ...
      <div className="bg-white shadow rounded-lg">
        ...
        <div>
          {purchases.map((purchase) => (
            ...
            >
              <img
                src={`http://localhost:3000/${purchase.itemOption.item.images[0].path}`}
                className="w-16 h-16 object-cover rounded-lg mr-4"
              />
              <div className="flex-1">
                <h3 className="text-lg font-semibold">
                  {purchase.itemOption.item.item_name_kr}
                </h3>
                <p className="text-gray-500 text-sm">
                  {purchase.itemOption.item.item_name_en}
                </p>
                <p className="text-gray-500 text-sm">
                  {purchase.itemOption.option}
                </p>
              </div>
              <div className="text-right">
                ...
                <p className="text-red-500 text-sm">{purchase.status}</p>
              </div>
            </div>
          ))}
        </div>
